            ...
            <div class="row">
                ...
                @if (session('success_message'))
                    <div class="col-12">
                        <div class="alert alert-success alert-dismissible fade show" role="alert">
                            ...
                            <a class="text-white text-decoration-underline ms-2"
                               href="{{ url('photo', $data->id) }}">{{ __('misc.view_photo') }} <i
                                    class="bi-arrow-right ms-1"></i></a>
                ...
                <div class="col-md-7 mb-3">
                    ...
                    <div class="filer-input-dragDrop position-relative border d-none d-lg-block" id="draggable">
                        ...
                        <video id="video" width="{{config('video.preview_width')}}"  controls>
                            <source
                                src="{{url('files/video/preview/'.'preview', $data->preview)}}">
                        </video>
                    ...
                    <div class="card border-0">
                        ...
                        <div class="card-body p-0">
                            ...
                            <form method="POST" action="{{ url('update/video') }}" enctype="multipart/form-data"
                                ...
                                <div class="mb-3">
                                    <input type="text" required class="form-control tagin" id="tagInput"
                                           value="{{ $data->tags }}" name="tags" placeholder="{{ trans('misc.tags') }}">
                                    <small class="d-block">* {{ trans('misc.add_tags_guide') }}
                                        ({{trans('misc.maximum_tags', ['limit' => $settings->tags_limit ]) }})</small>
                                </div>
                                ...
                                <div class="form-floating mb-3">
                                    <select name="categories_id" class="form-select" id="input-category">
                                        ...
                                        @foreach (Categories::where('mode','on')->orderBy('name')->get() as $category)
                                            <option @if ($data->categories_id == $category->id) selected="selected"
                                ...
                                <!-- Start Form Group -->
                                    ...
                                    <div
                                        ...
                                        <input class="form-check-input"
                                               @if ($data->attribution_required == 'yes') checked
                                               @endif name="attribution_required" type="checkbox" value="yes"
                                ...
                                @else
                                    <div class="form-floating mb-3">
                                        <input type="number" @if ($settings->default_price_photos) readonly
                                               value="{{$settings->default_price_photos}}"
                                               @else value="{{ $data->price }}" @endif name="price"
                                               class="form-control onlyNumber" autocomplete="off" id="price"
                                               placeholder="{{ trans('misc.price') }}">
                                        ...
                                        @if (! $settings->default_price_photos)
                                            <div class="alert alert-primary">
                                                ...
                                                <ul class="list-unstyled">
                                                    <li>{{trans('misc.small_photo_price')}} {{ $settings->currency_position == 'left' ? $settings->currency_symbol : null }}
                                                        <span
                                                            id="s-price">{{ $data->price }}</span>{{ $settings->currency_position == 'right' ? $settings->currency_symbol : null }}
                                                    </li>
                                                    <li>{{trans('misc.medium_photo_price')}} {{ $settings->currency_position == 'left' ? $settings->currency_symbol : null }}
                                                        <span
                                                            id="m-price">{{ $data->price * 2 }}</span>{{ $settings->currency_position == 'right' ? $settings->currency_symbol : null }}
                                                    </li>
                                                    <li>{{trans('misc.large_photo_price')}} {{ $settings->currency_position == 'left' ? $settings->currency_symbol : null }}
                                                        <span
                                                            id="l-price">{{ $data->price * 3 }}</span>{{ $settings->currency_position == 'right' ? $settings->currency_symbol : null }}
                                                    </li>
                                                    <li>{{trans('misc.vector_photo_price')}} {{ $settings->currency_position == 'left' ? $settings->currency_symbol : null }}
                                                        <span
                                                            id="v-price">{{ $data->price * 4 }}</span>{{ $settings->currency_position == 'right' ? $settings->currency_symbol : null }}
                                                        <small>{{trans('misc.if_included')}}</small></li>
                                                </ul>
                                                <small>{{trans('misc.price_minimum')}} {{Helper::amountFormat($settings->min_sale_amount)}}
                                                    - {{trans('misc.price_maximum')}} {{Helper::amountFormat($settings->max_sale_amount)}}</small>
