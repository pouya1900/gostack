...
@section('css')
    ...
    <meta property="og:title"
          content="{{ $response->title.' - '.trans_choice('misc.photos_plural', 1 ).' #'.$response->id }}"/>
                    ...
                    <div class="d-flex">
                        ...
                        <div class="flex-grow-1 ms-3">
                            ...
                            <ul class="list-inline mt-2 fs-5">
                                <li class="list-inline-item me-3"><a class="btn-facebook-share" title="Facebook"
                                                                     href="https://www.facebook.com/sharer/sharer.php?u={{ url()->current() }}"
                                                                     target="_blank"><i class="fab fa-facebook"></i></a>
                                </li>
                                <li class="list-inline-item me-3"><a class="btn-twitter-share" title="Twitter"
                                                                     href="https://twitter.com/intent/tweet?url={{ url()->current() }}&text={{ e( $response->title ) }}"
                                                                     data-url="{{ url()->current() }}"
                                                                     target="_blank"><i class="fab fa-twitter"></i></a>
                                </li>
                                <li class="list-inline-item me-3"><a class="btn-pinterest-share" title="Pinterest"
                                                                     href="//www.pinterest.com/pin/create/button/?url={{ url()->current() }}&media={{url('files/preview/'.$stockImages[1]->resolution, $stockImages[1]->name)}}&description={{ e( $response->title ) }}"
                                                                     target="_blank"><i
                                            class="fab fa-pinterest"></i></a></li>
                                <li class="list-inline-item"><a class="btn-whatsapp-share" title="Whatsapp"
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="collectionsData">
                            @if ($collections->count() != 0)
                                @foreach ($collections as $collection )
                                    ...
                                    <div class="form-check mb-1">
                                        <input class="no-show form-check-input addImageCollection"
                                               data-image-id="{{$response->id}}"
                                               data-collection-id="{{$collection->id}}" name="checked"
                                               {{$checked}} type="checkbox" value="true"
                                               id="collectionCheck{{$collection->id}}">
                                        <label class="form-check-label text-overflow"
                                               for="collectionCheck{{$collection->id}}">
                                            {{$collection->title}}
                                        </label>
                                    </div>
                            ...
                            @else
                                <div class="d-block text-center no-collections mb-1 p-3 bg-warning rounded"><i
                                        class="bi bi-exclamation-circle me-1"></i> {{ trans('misc.no_have_collections') }}
                                </div>
                            @endif

                        </div><!-- collection data -->

                        <small class="note-add @if ($collections->count() == 0) display-none @endif"><i
                                class="bi bi-info-circle me-1"></i> {{ trans('misc.note_add_collections') }}</small>
                        ...
                        <form method="POST" action="" enctype="multipart/form-data" id="addCollectionForm" class="mt-3">
                            ...
                            <div class="form-floating mb-2">
                                <input type="text" class="form-control" name="title" id="titleCollection"
                                       id="titleCollection" placeholder="{{ trans('admin.title') }}">
                                <label for="titleCollection">{{ trans('admin.title') }}</label>
                            </div>

                            <div class="form-check form-switch">
                                <input class="form-check-input radio-bws" name="type" type="checkbox"
                                       id="flexSwitchCheckDefault">
                                <label class="form-check-label"
                                       for="flexSwitchCheckDefault">{{ trans('misc.private') }}</label>
                            </div>
                            ...
                            <div class="d-block text-center">
                                <button type="submit" class="btn btn-custom" id="addCollection">
                                    <i></i> {{ trans('misc.create_collection') }}</button>
                            </div>
                        </form>
        ...
        <div class="container-custom container pt-5">
            <div class="row">
                ...
                <div class="col-md-9">
                    ...
                    @php
                        $newWidth   = config('video.preview_width')*2;
                        $newHeight  = $newWidth / $response->dim;
                    @endphp
                    <div class="text-center mb-3">
                        <div
                            style="margin: 0 auto; background: url('{{asset('public/img/pixel.gif')}}') repeat center center; max-width:{{$newWidth}}px; max-height: {{$newHeight}}px">

                            <a href="{{ url('files/video/preview/'.$stockImages[1]->resolution, $response->preview) }}"
                               class="glightbox" style="cursor: zoom-in;">
                                <small class="video_duration">
                                    <span>{{$response->best}}</span>
                                    <span>{{intval($response->duration/60).":".$response->duration % 60}}</span>
                                </small>
                                <video class="video_preview img-fluid lazyload" muted width="100%"
                                       style="display: inline-block; width: {{$newWidth}}px"
                                       poster="{{url('files/preview/'.$newWidth."x".$newHeight, $response->thumbnail).'?type=videoThumbnail'}}"
                                       data-src="{{url('files/video/preview/'.$stockImages[1]->resolution, $response->preview)}}">
                                    <source
                                        src="{{url('files/video/preview/'.$stockImages[1]->resolution, $response->preview)}}"
                                    />
                                </video>
                ...
                <!-- Start Block -->
                    <div class="d-block mb-4">
                        ...
                        @for ($i = 0; $i < $countTags; ++$i)
                            <a href="{{url('tags', str_replace(' ', '_', trim($tags[$i]))) }}"
                               class="btn btn-sm bg-white border e-none btn-category mb-2">
                                {{ $tags[$i] }}
                            </a>
                        @endfor
                    </div><!-- End Block -->
                ...
                <!-- Start Block -->
                    <div class="d-block mb-5">
                        ...
                        @if ($response->comments()->count() != 0 || auth()->check() && $settings->comments)
                            <h5 class="fw-light mb-3">{{trans('misc.comments')}} (<span
                                    id="totalComments">{{ number_format($response->comments()->count()) }}</span>)</h5>
                        @endif
            ...
            <span class="float-start me-2">
                                    ...
                                    <form action="{{ url('comment/store') }}" method="post" id="commentsForm">
                                        <div class="form-group text-form mb-2">
                                            ...
                                            <textarea name="comment" rows="3" required="required" min="2" id="comments"
                                                      class="form-control textarea-comments mentions-textarea"></textarea>
                                        </div>

                                        <!-- Alert -->
                                        <div class="alert alert-danger alert-small display-none"
                                             id="dangerAlertComments">
                                            <ul class="list-unstyled m-0" id="showErrorsComments"></ul>
                                        </div><!-- Alert -->

                                        <div class="form-group ">
                                            <button type="submit" class="btn btn-custom"
                                                    id="commentSend">{{ trans('auth.send') }}</button>
                                        </div>
                                    </form>
                                </div><!-- media body -->
                            </div><!-- media -->
                ...
                <div class="col-md-3">
                    ...
                    @if (auth()->check() &&  isset($response->user()->id) && auth()->id() == $response->user()->id)
                        <div class="row mb-3">
                            ...
                            <div class="col-md-12">
                                <a class="btn btn-sm bg-white border e-none btn-category d-block mb-2"
                                   href="{{ url('edit/video',$response->id) }}">{{trans('admin.edit')}}</a>
                            </div>
                            <div class="col-md-12">
                                <form method="POST" action="{{ url('delete/video', $response->id) }}"
                                      accept-charset="UTF-8" class="d-inline">
                                    @csrf
                                    <button type="button"
                                            class="btn btn-sm bg-white border e-none btn-category text-danger d-block w-100"
                                            id="deletePhoto">
                ...
                <!-- Start Panel -->
                        ...
                        <div class="panel-body">
                            <div class="media none-overflow">
                                ...
                                <div class="media-body">
                                    ...
                                    <small
                                        class="d-block text-muted">{{ number_format(User::totalImages( $response->user()->id))}} {{trans_choice('misc.images_plural', User::totalImages( $response->user()->id ))}}</small>
                                    ...
                                    <p class="mt-2">
                                        ...
                                        @if (auth()->check() && $response->user()->id != auth()->id() && $response->user()->paypal_account != '' || auth()->guest()  && $response->user()->paypal_account != '')
                                            <button type="button"
                                                    class="btn btn-sm bg-white border e-none btn-category showTooltip"
                                                    id="btnFormPP" title="{{trans('misc.buy_coffee')}}">
                                                <i class="bi bi-paypal"
                                                   style="color: #003087"></i> @guest {{trans('misc.coffee')}} @endguest
                                            </button>
                                        @endif
                                    </p>
                                </div>
                    ...
                    @if (auth()->check() && $response->status == 'active')
                        <div class="row mb-4">
                            ...
                            <div class="col-6">
                                @if (auth()->check())
                                    <a href="#" class="btn-collection" data-bs-toggle="modal"
                                       data-bs-target="#collections">
                                        <h3 class="d-block text-center margin-top-10"><i class="bi bi-plus-square"></i>
                                        </h3>
                                        <small
                                            class="d-block text-center text-muted">{{trans('misc.collection')}}</small>
                                    </a>
                                @endif
                            </div><!-- col-xs-6 -->
                        </div>
                    ...
                    <div class="card mb-4">
                        <div class="card-body p-0">
                            <ul class="list-stats list-inline">
                                <li>
                                    <h5 class="d-block text-center m-0"><i class="bi bi-eye me-1"></i> <small
                                            class="text-muted">{{Helper::formatNumber($response->visits()->count())}}</small>
                                    </h5>
                                </li>

                                <li>
                                    <h5 class="d-block text-center m-0"><i class="bi bi-heart me-1"></i> <small
                                            id="countLikes"
                                            class="text-muted">{{Helper::formatNumber($response->likes()->count())}}</small>
                                    </h5>
                                </li>

                                <li>
                                    <h5 class="d-block text-center m-0"><i class="bi bi-download me-1"></i> <small
                                            class="text-muted">{{Helper::formatNumber($response->downloads()->count())}}</small>
                                    </h5>
                                </li>
                ...
                <!-- btn-group -->
                                        ...
                                        @foreach( $stockImages as $stock )
                                            ...
                                            switch ($stock->type) {
                                                case 'hd':
                                                    ...
                                                    break;
                                                case 'fhd':
                                                    $_size = trans('misc.fhd');
                                                    break;
                                                case 'qhd':
                                                    $_size = trans('misc.qhd');
                                                    break;
                                                case 'uhd':
                                                    $_size = trans('misc.uhd');
                                                    break;
                                            }
                                            ?>
                                            <label class="list-group-item d-flex gap-2 c-pointer">
                                                <input class="form-check-input flex-shrink-0" type="radio"
                                                       @if ($stock->type == 'small') checked @endif name="type"
                                                       id="radio{{$stock->type}}" value="{{$stock->type}}">
                                                <span>
                  ...
                  <small class="d-block text-muted">{{trans('misc.file_size')}}: {{$stock->size}}</small>
                                ...
                                <!-- btn-free -->
                                <button type="submit"
                                        class="btn btn-lg btn-custom w-100 d-block @if ($settings->downloads == 'all' || $settings->downloads == 'users' && auth()->check()) btnDownload downloadableButton @endif"
                                        id="downloadBtn">
                        ...
                        @elseif (auth()->check()
                            ...
                            <form action="{{url('subscription/stock', $stockImages[2]->token)}}" method="post"
                                  id="formBuy">
                                @csrf
                                    ...
                                    @if ($settings->link_license != '')
                                        <small class="text-center d-block mt-1">
                                            <a href="{{$settings->link_license}}"
                                               target="_blank">{{trans('misc.view_license_details')}} <i
               ...
               <small class="d-block text-muted">{{trans('misc.file_size')}}: {{$stock->size}}</small>
                                ...
                                <!-- btn-sale -->
                                <button class="btn btn-custom btn-lg d-block w-100 downloadableButton" data-type="small"
                                    ...
                                    <div class="list-group mx-0">
                                        @foreach( $stockImages as $stock )
                                            ...
                                            switch ($stock->type) {
                                                case 'hd':
                                                    $_size = trans('misc.hd');
                                                    $imageType = trans('misc.hd');
                                                    break;
                                                case 'fhd':
                                                    $_size = trans('misc.fhd');
                                                    $imageType = trans('misc.fhd');
                                                    break;
                                                case 'qhd':
                                                    $_size = trans('misc.qhd');
                                                    $imageType = trans('misc.qhd');
                                                    break;
                                                case 'uhd':
                                                    $_size = trans('misc.uhd');
                                                    $imageType = trans('misc.uhd');
                                                    break;
                                            }
                                            ?>
                                            <label class="list-group-item d-flex gap-2 c-pointer itemPrice"
                                                   data-type="{{$stock->type}}" data-image="{{$imageType}}"
            ...
            <small class="d-block text-muted">{{trans('misc.file_size')}}: {{$stock->size}}</small>
                                ...
                                <!-- btn-sale -->
                                <button class="btn btn-custom btn-lg d-block w-100" data-type="small" id="downloadBtn"
                                        @if (auth()->check() && auth()->id() != $response->user_id) data-bs-toggle="modal"
                                        data-bs-target="#checkout" type="button" @else type="submit" @endif>
                                    ...
                                    @else
                                        ...
                                        <span id="priceItem">{{ $settings->currency_position == 'left' ? $settings->currency_symbol : null }}<span
                                                id="itemPrice">{{$itemPrice}}</span>{{ $settings->currency_position == 'right' ? $settings->currency_symbol : null }} <small
                            ...
                            @if ($settings->downloads == 'all' && $response->item_for_sale == 'free')
                                <small class="d-block w-100 text-center mt-2 lh-sm fs-small text-muted">
                                    ...
                                    <a href="https://policies.google.com/privacy"
                                       target="_blank">{{trans('misc.privacy')}}</a> - <a
                                        href="https://policies.google.com/terms"
                                        target="_blank">{{trans('misc.terms')}}</a>
                                </small>
                            @endif
                        @endguest
                            ...
                            <div class="card-body">
                                ...
                                @if( $response->attribution_required == 'yes' )
                                    <small class="d-block text-muted"><i
                                            class="bi bi-check2 me-1"></i> {{trans('misc.attribution_required')}}
                                    </small>
                                @else
                                    <small class="d-block text-muted"><i
                                            class="bi bi-x me-1"></i> {{trans('misc.no_attribution_required')}}</small>
                                @endif
                    ...
                    <!-- Start Panel -->
                        <div class="card mb-4">
                            <div class="card-body">
                                <h6 class="float-start m-0" style="line-height: inherit;"><i
                                        class="bi bi-share me-1"></i> {{trans('misc.share')}}</h6>

                                <ul class="list-inline float-end m-0 fs-5">
                                    <li class="list-inline-item"><a class="btn-facebook-share" title="Facebook"
                                                                    href="https://www.facebook.com/sharer/sharer.php?u={{ url()->current() }}"
                                                                    target="_blank"><i class="fab fa-facebook"></i></a>
                                    </li>
                                    <li class="list-inline-item"><a class="btn-twitter-share" title="Twitter"
                                                                    href="https://twitter.com/intent/tweet?url={{ url()->current() }}&text={{ e( $response->title ) }}"
                                                                    data-url="{{ url()->current() }}" target="_blank"><i
                                                class="fab fa-twitter"></i></a></li>
                                    <li class="list-inline-item"><a class="btn-pinterest-share" title="Pinterest"
                                                                    href="//www.pinterest.com/pin/create/button/?url={{ url()->current() }}&media={{url('files/preview/'.$stockImages[1]->resolution, $stockImages[1]->name)}}&description={{ e( $response->title ) }}"
                        ...
                        <div class="card mb-4">
                            <div class="card-body">
                                ...
                                @if ($response->camera != '')
                                    ...
                                    <small class="d-block text-muted"><a
                                ...
                                @if ($response->date_time_original)
                                    <small
                                        class="d-block text-muted mt-2">{{trans('admin.original_date_taken')}}</small>
                                    <small
                                        class="d-block text-muted"><strong>{{ Helper::formatDate($response->date_time_original) }}</strong></small>
                                @endif
                    ...
                    <ul class="list-group">
                        <li class="list-group-item"><i class="bi bi-info-circle me-1"></i>
                            <strong>{{trans('misc.details')}}</strong></li>

                        <li class="list-group-item">{{trans('misc.published')}} <small
                                class="float-end text-muted">{{Helper::formatDate($response->date)}}</small></li>
                        <li class="list-group-item">{{trans('misc.image_type')}} <small
                                class="float-end text-muted">{{strtoupper($response->extension)}} </small></li>
                        <li class="list-group-item">{{trans('misc.resolution')}} <small
                                class="float-end text-muted">{{$stockImages[2]->resolution}}</small></li>
                        <li class="list-group-item">{{trans('misc.category')}}
                            <small class="float-end text-muted">
                                <a href="{{url('category',$response->category->slug)}}"
                                   title="{{ Lang::has('categories.' . $response->category->slug) ? __('categories.' . $response->category->slug) : $response->category->name }}">
                        ...
                        <li href="#" class="list-group-item">{{trans('misc.file_size')}} <small
                                class="float-end text-muted">{{$stockImages[2]->size}}</small></li>
                    </ul>
                            ...
                            <div class="modal-dialog modal-sm">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <!-- form start -->
                                        <form method="POST" action="{{ url('report/photo') }}"
                                              enctype="multipart/form-data" id="formReport">
                                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                            ...
                                            <button type="submit" class="btn btn-custom float-end"
                    ...
                    @if (auth()->check() && auth()->id() != $response->user()->id && $response->status == 'active')
                        <div class="d-block text-center mt-2">
                            <a href="#" data-bs-toggle="modal" data-bs-target="#reportImage" class="text-danger"><i
                                    class="bi bi-exclamation-triangle me-1"></i> {{ trans('misc.report_photo') }}</a>
                        </div>
                    @endif
                            ...
                            <div class="container">
                                <div class="row">
                                    ...
                                    <div class="col-md-6 ps-0">
                                        ...
                                        <form method="post" action="{{url('buy/stock', $stockImages[2]->token)}}"
                                              class="d-inline" id="formSendBuy">
                                            @csrf
                                            ...
                                            @foreach (PaymentGateways::where('enabled', '1')->orderBy('type', 'DESC')->get() as $payment)
                                                <div class="form-check custom-radio mb-2">
                                                    <input name="payment_gateway" value="{{$payment->id}}"
                                                           ...
                                                           @if ($paymentsGatewaysEnabled == 1 && auth()->user()->funds == 0.00) checked
                                                           @endif class="form-check-input radio-bws" type="radio">
                                            ...
                                            <div class="form-check custom-radio mb-3">
                                                <input name="payment_gateway"
                                                       @if (auth()->user()->funds == 0.00) disabled
                                                       @endif value="wallet" id="wallet"
                                                       class="form-check-input radio-bws" type="radio">
                                                <label class="form-check-label" for="wallet">
                                                    <span><img class="me-1 rounded"
                                                               src="{{ url('public/img/payments/wallet.png') }}"
                                                               width="20"/> <strong>{{ __('misc.wallet') }}</strong></span>
                                                    <small class="w-100 d-block">
                                    ...
                                    <div class="col-md-6 ps-0">
                                            ...
                                            <li class="list-group-item py-1 px-0">
                                                <div class="row">
                                                    ...
                                                    <div class="col-auto fw-bold">
                                                        <small>{{ $settings->currency_position == 'left' ? $settings->currency_symbol : null }}
                                                            <span
                                                                id="total">{{ Helper::amountGross($itemPrice) }}</span>{{ $settings->currency_position == 'right' ? $settings->currency_symbol : null }} {{ $settings->currency_code }}
                                                        </small>
                                                    </div>
                                                </div>
                                            </li>
                                        ...
                                        <button type="submit"
                                                @if ($paymentsGatewaysEnabled == 1 && auth()->user()->funds == 0.00) @else disabled
                                                @endif class="btn btn-success w-100" id="payButton">
                                            <i></i> {{ __('misc.pay') }}</button>
                                        <div class="w-100 d-block text-center">
                                            <button type="button"
                                                    class="btn btn-link e-none text-decoration-none text-reset"
